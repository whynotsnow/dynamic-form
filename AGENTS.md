...
- 保持改动范围聚焦，不做无关重构。
- 提交代码时必须使用本文档记录的本机 Git 作者信息：`whynotsnow <whynotsnow@163.com>`。
- 不要伪造提交身份，不要使用 `Codex <codex@openai.com>` 或其他 Agent 身份提交。
- 提交时直接使用上述作者信息；不要每次重新查询本机 Git 作者和邮箱，除非用户明确要求刷新本文档记录。
