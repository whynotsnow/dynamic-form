...
本文固定 DynamicForm 独立文档站的建设方案。当前目标不是立即重构仓库，而是在不影响库构建和发布流程的前提下，把 `docs/` 和 `demos/` 逐步组织成可独立部署的站点。
...
- 根目录 `npm run build` 使用 `tsup` 构建库产物，不负责构建文档站。

站点建设应优先复用现有内容，不应为了官网先迁移全部源码目录。

### 建议架构

采用阶段式 monorepo 方向，但第一阶段只新增独立站点应用：

```text
dynamic-form/
├── apps/
│   └── docs-site/           # 独立文档站，第一阶段新增
├── docs/                    # 继续保存双语 Markdown 文档
├── demos/                   # 继续保存 demo 组件和 demoRegistry
├── src/                     # 继续保存库源码
└── package.json             # 后续加入 workspace 和站点命令
```

完整 monorepo 可作为后续阶段目标：

```text
dynamic-form/
├── packages/
│   └── dynamic-form/        # 未来迁移：库源码、tests、tsup 配置
├── apps/
│   └── docs-site/           # 官网、文档、playground
└── package.json             # npm workspaces 统一调度
```

第一阶段不移动 `src/`、`docs/`、`demos/`，避免影响现有 npm 包构建和发布路径。
...
This document records the plan for a standalone DynamicForm documentation site. The current goal is not to immediately restructure the repository, but to gradually turn `docs/` and `demos/` into a separately deployable site without disrupting the library build and release flow.

### Background

The repository already has the foundation for a documentation site:

- `src/` contains the dynamic form library source.
...
- Root `npm run build` uses `tsup` to build the library package and does not build a documentation site.

The site should reuse existing content first. It should not require moving the entire source tree before the site exists.

### Recommended Architecture

Use a staged monorepo direction, but only add a standalone site app in the first phase:

```text
dynamic-form/
├── apps/
│   └── docs-site/           # Standalone docs site, added in phase 1
├── docs/                    # Existing bilingual Markdown docs
├── demos/                   # Existing demo components and demoRegistry
├── src/                     # Existing library source
└── package.json             # Later adds workspace and site commands
```

A full monorepo can remain a later target:

```text
dynamic-form/
├── packages/
│   └── dynamic-form/        # Future migration: source, tests, tsup config
├── apps/
│   └── docs-site/           # Website, docs, playground
└── package.json             # npm workspaces orchestration
```

Do not move `src/`, `docs/`, or `demos/` in the first phase. This keeps the existing npm package build and release paths stable.
